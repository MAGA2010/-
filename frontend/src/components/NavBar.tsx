    ...
    <header className="sticky top-0 z-50 border-b border-line/60 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/80">
      <div className="mx-auto flex h-14 max-w-7xl items-center gap-4 px-4 sm:px-6">
          ...
          {NAV_LINKS.map((link) => {
            ...
            return (
              <Link
                ...
                className={
                  "whitespace-nowrap rounded-md px-2.5 py-1.5 text-[13px] font-medium transition-colors lg:px-3 lg:text-sm " +
                  (isActive
                    ? "bg-ink/10 text-ink"
        ...
        <div className="ml-auto flex items-center gap-3">
          <Link
            href="/match"
            className="hidden whitespace-nowrap rounded-lg bg-ink px-4 py-1.5 text-sm font-semibold text-panel shadow-sm transition hover:bg-ink/90 sm:inline-flex"
          >
            开始自主测验
          </Link>
          {/* Mobile menu toggle */}
