...
import { buildCityAggregates, getCitiesByState, getStateAbbr, getStateCenter, getCityMetricDisplay, getCityMetricValue } from "@/lib/city-utils";
...
function normalizeStateSearch(value: string): string {
  return value
    .trim()
    .toLowerCase()
    .replace(/state|州/g, "")
...
function stateMatchesSearch(
  state: { fipsCode: string; name: string; nameEn: string },
  query: string,
): boolean {
  const normalizedQuery = normalizeStateSearch(query);
  ...
  const candidates = [
    ...
    state.nameEn,
    `${state.nameEn} State`,
    getStateAbbr(state.fipsCode),
    state.fipsCode,
    ...(STATE_SEARCH_ALIASES[state.fipsCode] ?? []),
  ];
  return candidates.some((candidate) => normalizeStateSearch(candidate).includes(normalizedQuery));
}
...
export function MapShell({
 ...
 const flyToDefault = useCallback(() => {
  ...
  const filteredStateOptions = useMemo(
    () => STATE_OPTIONS.filter((state) => stateMatchesSearch(state, stateSearch)),
    [stateSearch],
  );
    ...
    >
      ...
      <div className="relative flex flex-1 flex-col min-w-0">
        ...
        <div className="relative flex flex-col flex-1 min-h-0">
          ...
          <div className="absolute right-3 top-3 z-10 flex max-w-[280px] flex-col items-end gap-1.5">
            {/* Reset view — always visible on screen */}
            <button
              type="button"
              onClick={flyToDefault}
              className="inline-flex w-full items-center justify-center gap-1.5 rounded-full border border-line bg-white/88 px-2.5 py-1 text-[11px] font-medium text-ink/64 shadow-sm backdrop-blur transition-colors hover:bg-white hover:text-ink"
              title="一键复原 — 回到初始地图视图"
            >
            ...
            <div className="relative">
              <button
                type="button"
                onClick={() => {
                  setShowStateDropdown((visible) => !visible);
                  setStateSearch("");
                }}
              className={`max-w-full whitespace-nowrap rounded-full border px-2.5 py-1 text-[11px] font-medium shadow-sm backdrop-blur transition-colors ${
                cityDrilldownEnabled
              ...
              {showStateDropdown && (
                <div className="absolute right-0 top-full mt-1 z-20 flex w-[300px] max-w-[calc(100vw-1.5rem)] flex-col overflow-hidden rounded-xl border border-line bg-white shadow-lg backdrop-blur-sm">
                  ...
                  <div className="border-b border-line/50 p-2">
                    <label className="relative block">
                      <span className="sr-only">搜索州中文名、英文名或缩写</span>
                      <Search className="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2 text-ink/32" size={14} />
                      <input
                        type="search"
                        value={stateSearch}
                        onChange={(event) => setStateSearch(event.target.value)}
                        placeholder="加州 / California / CA"
                        autoComplete="off"
                        className="h-9 w-full rounded-lg border border-line/60 bg-paper/70 pl-8 pr-8 text-xs text-ink outline-none placeholder:text-ink/30 focus:border-cobalt/60"
                      />
                      {stateSearch && (
                        <button
                          type="button"
                          ...
                          aria-label="清除州搜索"
                          className="absolute right-2 top-1/2 grid h-5 w-5 -translate-y-1/2 place-items-center rounded text-ink/34 transition hover:bg-ink/5 hover:text-ink"
                        >
                          <X size={12} />
                        </button>
                      )}
                    </label>
                    <div className="mt-1.5 flex items-center justify-between px-0.5 text-[9px] text-ink/34">
                      <span>中文 · English · 缩写 · FIPS</span>
                      <span>{filteredStateOptions.length} 个结果</span>
                    </div>
                  </div>
                  <div className="max-h-[250px] overflow-y-auto py-1">
                    {filteredStateOptions.length > 0 ? filteredStateOptions.map((st) => {
                      const abbreviation = getStateAbbr(st.fipsCode);
                      return (
                        <button
                          key={st.fipsCode}
                          type="button"
                          onClick={() => handleStateSelect(st.fipsCode)}
                          className={`flex w-full items-center gap-2.5 px-3 py-2 text-left text-xs transition-colors hover:bg-cobalt/8 ${
                            selectedStateFips === st.fipsCode ? "bg-cobalt/10 text-cobalt font-medium" : "text-ink"
                          }`}
                        >
                          <span className="grid h-6 w-8 shrink-0 place-items-center rounded bg-ink/5 text-[10px] font-semibold text-ink/52">{abbreviation}</span>
                          <span className="min-w-0 flex-1">
                            <span className="block truncate">{st.name}州</span>
                            <span className="mt-0.5 block truncate text-[10px] font-normal text-ink/38">{st.nameEn}</span>
                          </span>
                          <span className="shrink-0 text-[9px] font-normal text-ink/24">{st.fipsCode}</span>
                        </button>
                      );
                    }) : (
                      <div className="px-4 py-7 text-center">
                        <Search className="mx-auto text-ink/18" size={20} />
                        <p className="mt-2 text-xs font-medium text-ink/48">没有找到这个州</p>
                        <p className="mt-1 text-[10px] text-ink/30">试试中文名、英文名或两位缩写</p>
                      </div>
                    )}
                  </div>
                </div>
            ...
            <span className="pointer-events-none max-w-full whitespace-nowrap rounded-full border border-line bg-white/88 px-2.5 py-1 text-[11px] font-medium text-ink/64 backdrop-blur">
         ...
         {/* Map legend overlay */}
          <div className="absolute bottom-4 left-4 z-10 flex flex-col gap-3 pointer-events-none">
            ...
            {(selectedCityId || selectedStateFips) && (
              <button
                type="button"
                onClick={flyToDefault}
                className="pointer-events-auto inline-flex items-center gap-1.5 rounded-full border border-line/70 bg-white/92 px-3.5 py-2 text-xs font-medium text-ink shadow-sm backdrop-blur transition hover:bg-white hover:border-ink/30 hover:shadow-md active:scale-[0.97]"
              >
